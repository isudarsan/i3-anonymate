...
class CustomIpAddressRecognizer(IpRecognizer, CustomRecognizerMixin):
    """
    Recognize IP addresses using Presidio´s pre-defined recognizer.
    """

    POSSIBLE_LANGUAGES = None
    POSSIBLE_ENTITIES = [constants.ENTITY_IP_ADDRESS]
    POSSIBLE_REGIONS = constants.VALID_GLOBALLY

    def __init__(self, supported_language, supported_entities, supported_regions):
        IpRecognizer.__init__(
            self,
